...
template <class T>
WellFlow<T>::WellFlow(const string fileName)
{
	loadTask(fileName);
	...
	well = new Well<T>(props.r1, props.r2, props.K, props.rw);
...
template <class T>
void WellFlow<T>::loadTask(const string fileName)
{
...
template <class T>
void WellFlow<T>::setSummator(BaseSum<T>* _inclSum)
{
	inclSum = _inclSum;
}

template <class T>
const Parameters<T>* WellFlow<T>::getProps() const
{
	return &props;
}

template <class T>
const Well<T>* WellFlow<T>::getWell() const
{
	return well;
}

template <class T>
void WellFlow<T>::findRateDistribution()
{
	...
	// Set rate deviation
	auto setRateDev = [this](int seg_idx, T ratio) {
		well->segs[seg_idx].rate += props.rate * ratio;
	};

	// Fills dpdq matrix
	auto fill_dpdq = [&, this](T mult) {
		T p1, p2, ratio;
		ratio = mult * 0.0001 / (T)(props.K);
...
template <class T>
void WellFlow<T>::calcPressure()
{
	well->pres_av = well->pres_dev = 0.0;

	for (int k = 0; k < props.K; k++)
	{
		WellSegment<T>& seg = well->segs[k];
...
template class WellFlow<float>;
template class WellFlow<double>;
