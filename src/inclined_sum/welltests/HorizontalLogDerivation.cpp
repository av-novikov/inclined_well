	...
	for (int arr_idx = 0; arr_idx < props->K * props->K; arr_idx++)
	{
		...
		for (int m = 1; m < props->M; m++)
		{
			...
			for (int n = 1; n < props->N; n++)
			{
				...
				{
					lambda = M_PI * M_PI * props->kx / props->visc / Parameters::porosity / Parameters::compressibility * 
...
double HorizontalLogDerivation::get3D(int seg_idx)
{
	...
	sum *= (-8.0 / M_PI / props->sizes.y / props->sizes.z / Parameters::porosity / Parameters::compressibility);
