...
void Inclined3dSum::prepare3D()
{
	...
	auto intExpansion2d = [this](double A, double B) {
		return -2.0 / sqrt(M_PI) * (pow(B / sqrt(A), 1.5) * tgamma(3.0 / 4.0, A / props->xi_c) * 
									(A*A - A*B*B / 4.0 + 21.0 / 160.0 * B*B*B*B) + 
									B*B/A/A / 480.0 * pow(B / sqrt(props->xi_c), 1.5) * exp(-A / sqrt(props->xi_c)) * 
									(4.0 * A * (3.0 * B*B / sqrt(props->xi_c) - 10) + 21.0 * B*B));
	};
	...
	for (int arr_idx = 0; arr_idx < props->K * props->K; arr_idx++)
	{
		...
		sum3 *= (props->visc * props->length / 4.0 / M_PI / props->kx);
