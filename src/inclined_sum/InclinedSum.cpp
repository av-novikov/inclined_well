...
double InclinedSum::getPres(const Point& r)
{
	...
	for (int k = 0; k < sprops.K; k++)
	{
		...
		for (int m = 1; m <= sprops.M; m++)
		{
			...
			for (int i = -sprops.I; i <= sprops.I; i++)
			{
	...
	sum2d *= (props->visc * props->sizes.x / M_PI / M_PI / props->sizes.z / props->kx / sin(gprops->alpha));
	sum3d *= (2.0 * props->visc / M_PI / props->sizes.x / props->sizes.z / props->kx / cos(gprops->alpha));
